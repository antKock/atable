...
// Caractérisation écran foyer — hub « Toi + Tes carnets » + détail de foyer.
...
test("hub → détail : rename inline, invitation (code + lien), dernier membre = pas de « Quitter »", async ({
  ...
  // foyer — arbitrage 2026-07), seule « Supprimer le carnet » reste (la
  // suppression + déconnexion est couverte par le test suivant).
  await expect(page.getByRole("button", { name: "Quitter ce carnet" })).toHaveCount(0);
  await expect(page.getByRole("button", { name: "Supprimer le carnet" })).toBeVisible();
...
test("détail : suppression avec double confirmation → foyer effacé", async ({
  ...
  await expect(dialog.getByText("Supprimer le carnet ?")).toBeVisible();
