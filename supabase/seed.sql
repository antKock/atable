...
INSERT INTO recipes (id, title, ingredients, steps, photo_url, household_id, is_seed, created_at)
VALUES
  (
    gen_random_uuid(),
...
-- ---------------------------------------------------------------------------
-- Demo recipe tags (relational model — the legacy recipes.tags TEXT[] column
-- was dropped in migration 018).
-- Two statements: tags created by a data-modifying CTE would not be visible
-- to the linking SELECT within the same statement.
-- ---------------------------------------------------------------------------
...
WHERE NOT EXISTS (
  ...
  WHERE lower(tg.name) = lower(m.tag_name)
    AND (tg.household_id IS NULL
         OR tg.household_id = '00000000-0000-0000-0000-000000000000'::uuid)
)
