...
| Taille max du body | Garde applicative `content-length` → 413 (`src/lib/body-limit.ts`, warning Sentry par route). Réponse 413 : `{ code: "BODY_TOO_LARGE", maxBytes }` (le front peut brancher dessus). **Le middleware Traefik `buffering` posé le 2026-09-06 a été retiré le 2026-09-13** : `vulcand/oxy/buffer` renvoie un 500 « Internal Server Error » (corps de 21 octets, rien dans les logs Next ni Sentry) sur **toute réponse à corps vide d'un route handler** — `DELETE /api/auth/session` (déconnexion), `/api/auth/session/clear`, 405 sur `POST /`… — vécu par Anthony en se déconnectant, remonté par les logs Traefik (`failed to read response, err: no data ready`, 19 occurrences depuis le 06/09). Config sauvegardée avant retrait (`/tmp/traefik-{prod,staging}-before.yml` le jour même ; la relire avec `application.readTraefikConfig`). Conséquence assumée : un corps `chunked` (sans `content-length`) échappe à la garde applicative ; ne pas reposer `buffering` sans vérifier ce cas. Les 5xx Traefik sont désormais surveillés (`docs/infra/veilleur-ops.md`) |
