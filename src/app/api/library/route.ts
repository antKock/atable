...
export const GET = withHouseholdAuth(async (_request, _ctx, { householdId }) => {
  ...
  const [recipesResult, tagsResult] = await Promise.all([
    supabase
      .from("recipes")
      .select(
        "id, title, ingredients, photo_url, created_at, generated_image_url, enrichment_status, image_status, prep_time, cook_time, cost, seasons, recipe_tags(tag_id, tags(id, name, category))",
      )
