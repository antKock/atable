...
export default async function HouseholdDetailPage({ params }: Props) {
  ...
  if (membership.isDemo) {
    ...
    members = [
      { ownerId: owner.ownerId, displayName: viewerName, role: membership.role, isViewer: true },
    ];
  } else {
    ...
    if (membersError) {
      throw new Error(
        `household detail: chargement des membres impossible (${membersError.message})`,
      );
    }
