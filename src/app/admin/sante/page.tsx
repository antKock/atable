      ...
      <div className="page">
        <div className="section" style={{ marginTop: 8 }}>
          <SectionHead
            n="S"
            title="Santé"
            q="Cadence quotidienne, 30 secondes : quelque chose est-il cassé ou anormalement cher ?"
          />
          <div className="cards">
            <Light
              ok={o.health.pipeline.ok}
              label="Pipeline IA"
              detail={o.health.pipeline.detail}
            />
        ...
        <div className="section">
          <SectionHead
            n="1"
            title="Pipeline d'enrichissement"
            q="Recettes créées sur 28 j, échecs à relancer via batch-enrich"
          />
          <div className="cards">
            <Card span={4} title="Sur 28 jours" sub="Hors démo">
              <BigStats
                stats={[
                  {
                    value: `${ops.pipeline.rate} %`,
                    label: `enrichies (${ops.pipeline.enriched} / ${ops.pipeline.created})`,
                  },
                  {
                    value: ops.pipeline.stale,
                    label: "bloquées depuis + 1 h (pending / processing)",
                  },
                ]}
              />
            </Card>
        ...
        <div className="section">
          ...
          <div className="cards">
            <Card
              span={4}
              title="Coût IA · 28 j"
              sub="Hors démo · réconcilié avec la facture OpenAI"
            >
              <BigStats
                stats={[
                  {
                    value: usd(ops.cost.total),
                    label:
                      ops.cost.billed != null
                        ? `instrumenté · facturé ${usd(ops.cost.billed)} (org. entière)`
                        : "instrumenté (pas de clé admin OpenAI)",
                    hint: `${ops.cost.aiCalls} appels · ${usd(ops.cost.perRecipe)} / recette`,
                  },
                  {
                    value: usd(ops.cost.demo),
                    label: "consommé par la démo",
                    hint: `${ops.demo.aiCalls} appels IA depuis la démo`,
                  },
                ]}
              />
            </Card>
            <Card span={4} title="Crons" sub="Derniers passages · alerte au-delà de 36 h">
              <BigStats
                stats={[
                  {
                    value: <span style={{ fontSize: 20 }}>{ops.crons.rollupLabel}</span>,
                    label: "demo-reset (03:00 UTC) · rollup stats_daily",
                  },
                  {
                    value: <span style={{ fontSize: 20 }}>{ops.crons.syncLabel}</span>,
                    label: "app-store-sync (10:00 UTC)",
                  },
                ]}
              />
            </Card>
                ...
                stats={[
                  {
                    value: (
                      <>
                        {ops.demo.seedFr} <small>FR</small> · {ops.demo.seedEn} <small>EN</small>
                      </>
                    ),
                    label: "recettes seed en place",
                  },
                  {
                    value: ops.demo.trials,
                    label: "essais démo (sondes exclues)",
                    hint: `${ops.demo.frozen} blocages « monde gelé » (403) · ${ops.demo.recipes} recettes ajoutées puis purgées`,
                  },
                ]}
              />
            </Card>
                ...
                stats={[
                  {
                    value: (
                      <>
                        {ops.account.withEmail} <small>/ {ops.account.total}</small>
                      </>
                    ),
                    label: `personnes avec un e-mail de secours · ${ops.account.newWithEmail} / ${ops.account.newTotal} parmi les nouvelles`,
                    hint: `${ops.account.recoverySent} tokens émis · ${ops.account.recoveryUsed} consommés · ${ops.account.burned} brûlé${ops.account.burned > 1 ? "s" : ""} (5 essais) · ${ops.account.merges} fusion${ops.account.merges > 1 ? "s" : ""} d'identités`,
                  },
                ]}
              />
            </Card>
