...
type TipProps = Partial<TooltipContentProps<TooltipValueType, string | number>> & {
  suffix?: string;
};

const fr = (n: unknown) => (typeof n === "number" ? n.toLocaleString("fr-FR") : String(n ?? ""));

function Tip({ active, payload, label, suffix }: TipProps) {
    ...
    >
      ...
      {payload
        ...
        .map((p, i) => (
          <div
            key={i}
            style={{ display: "flex", alignItems: "center", gap: 8, marginTop: i ? 5 : 0 }}
          >
            <span
              style={{
                width: 9,
            ...
            />
            <span style={{ fontFamily: FONT, fontSize: 12.5, color: P.muted, flex: 1 }}>
              {p.name}
            </span>
...
const total = (rows: Row[], keys: string[]) =>
  rows.reduce((s, r) => s + keys.reduce((a, k) => a + (Number(r[k]) || 0), 0), 0);
...
export function NorthStarChart({
  ...
  if (values.every((v) => v === 0))
    return <Empty height={120} sub="Se remplit avec les jours actifs des personnes." />;
  return (
    <ResponsiveContainer width="100%" height={height}>
...
export function StackedWeekly({
  ...
  if (
    ...
    return <Empty height={height} sub={emptySub} />;
  return (
    <ResponsiveContainer width="100%" height={height}>
      <BarChart data={data} margin={{ top: 12, right: 8, left: -12, bottom: 0 }}>
        ...
        />
        <Tooltip
          content={<Tip suffix={percent ? " %" : ""} />}
          cursor={{ fill: "rgba(110,122,56,0.06)" }}
        />
        {series.map((s, i) => (
          <Bar
            key={s.key}
        ...
        {markerLabel && (
          <ReferenceLine
            ...
            stroke={P.faint}
            strokeDasharray="3 4"
            label={{ value: markerGlyph ?? "", position: "top", fontSize: 11, fill: P.muted }}
          />
        )}
      </BarChart>
    </ResponsiveContainer>
...
export function ActivationWeekly({
  ...
  data: { label: string; arrivals: number; activated: number }[];
  height?: number;
}) {
  if (total(data, ["arrivals"]) === 0)
    return <Empty height={height} sub="Se remplit avec les arrivées jugeables à J+7." />;
  return (
    <ResponsiveContainer width="100%" height={height}>
...
export function RetentionCurves({
  ...
  curves: { month: string; label: string; points: (number | null)[] }[];
  refPct?: number;
  height?: number;
}) {
  if (curves.length === 0)
    return (
      <Empty height={height} sub="Une cohorte apparaît dès que son mois 1 est entièrement passé." />
    );
...
export function CohortCake({
  ...
  if (total(data, keys) === 0)
    return <Empty height={height} sub="Se remplit avec les jours actifs des personnes." />;
  return (
    <ResponsiveContainer width="100%" height={height}>
    ...
    <ResponsiveContainer width="100%" height={height}>
      <BarChart data={data} margin={{ top: 16, right: 8, left: -12, bottom: 0 }}>
        ...
        <Bar
          ...
          maxBarSize={44}
          isAnimationActive={false}
          label={{ position: "top", fontFamily: MONO, fontSize: 12, fill: P.ink }}
        />
      </BarChart>
    </ResponsiveContainer>
  );
...
export function StoreRates({
  ...
  if (data.every((d) => d.imprToDl == null && d.dlToOpen == null))
    return (
      <Empty
        height={height}
        sub="Se remplit avec les semaines de données Apple (depuis le 16 août)."
      />
    );
  return (
    <ResponsiveContainer width="100%" height={height}>
      <LineChart data={data} margin={{ top: 8, right: 12, left: -8, bottom: 0 }}>
        ...
        <Line
          ...
          type="monotone"
          dataKey="imprToDl"
          name="Impression → téléchargement (axe droit)"
          stroke={P.ochre}
          strokeWidth={2}
          dot={{ r: 3 }}
          connectNulls={false}
        ...
        <Line
          ...
          type="monotone"
          dataKey="dlToOpen"
          name="Téléchargement → 1ʳᵉ ouverture"
          stroke={P.olive}
          strokeWidth={2}
          dot={{ r: 3 }}
          connectNulls={false}
        ...
        <Line
          yAxisId="left"
          type="monotone"
          dataKey="openToCarnet"
          name="Ouverture → 1er carnet"
          stroke={P.terracotta}
          strokeWidth={2}
          dot={{ r: 3 }}
          connectNulls={false}
