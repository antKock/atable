...
type TipProps = Partial<TooltipContentProps<TooltipValueType, string | number>> & { suffix?: string };
...
const total = (rows: Row[], keys: string[]) => rows.reduce((s, r) => s + keys.reduce((a, k) => a + (Number(r[k]) || 0), 0), 0);
...
export function CohortCake({ data, keys, labels, height = 220 }: { data: Row[]; keys: string[]; labels: string[]; height?: number }) {
