...
export default function RecipeCarousel({ title, recipes }: RecipeCarouselProps) {
  ...
  return (
    <section role="region" aria-label={t.a11y.carousel(title)} className="overflow-x-clip">
      <h2 className="mb-3 px-4 text-lg font-semibold text-foreground">
        {title}
      </h2>
      <div className="flex gap-3 overflow-x-auto px-4 py-3 -my-3 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {recipes.map((recipe) => (
