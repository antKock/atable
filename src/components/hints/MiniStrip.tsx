...
type Props = {
  ...
  action: ReactNode;
  onDismiss?: () => void;
  /** Identifiant `data-track` (#28) du bandeau : impression + croix `.dismiss`. */
  track?: string;
};
...
export default function MiniStrip({ icon, label, action, onDismiss, track }: Props) {
  const t = useT();
  return (
    <div
    ...
    >
      ...
      {onDismiss ? (
        <button
          type="button"
          onClick={onDismiss}
          data-track={track ? `${track}.dismiss` : undefined}
