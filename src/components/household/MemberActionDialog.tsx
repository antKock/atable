...
export default function MemberActionDialog({ householdId, member, onClose }: Props) {
  ...
  async function changeRole(nextRole: MembershipRole) {
    if (!member) return
    void haptics.light()
    const data = await roleMutation.run(`/api/households/${householdId}/members/${member.ownerId}`, {
      method: 'PATCH',
      body: { role: nextRole },
    })
    if (!data) return
  ...
  async function remove() {
    if (!member) return
    void haptics.heavy()
    const data = await removeMutation.run(`/api/households/${householdId}/members/${member.ownerId}`, {
      method: 'DELETE',
    })
    if (!data) return
    onClose()
