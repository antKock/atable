import { NextResponse } from "next/server";
import * as Sentry from "@sentry/nextjs";
import { getOwnerContext } from "@/lib/auth/owner-context";

export type AuthContext = {
  householdId: string;
  sessionId: string | null;
};

export type WithHouseholdAuthOptions = {
  /**
   * 403 si l'owner est INVITÉ (role !== 'member') sur le foyer résolu. À poser
   * sur toute route de MUTATION household-scopée (Lot 3 — enforcement lecture
   * seule invité, 100 % applicatif car RLS sans policy). Les routes de LECTURE
   * l'omettent : un invité consulte les recettes en direct. Mono-appartenance
   * jusqu'au Lot 4 → le foyer résolu est toujours memberships[0].
   */
  requireMember?: boolean;
};

/**
 * Shared guard for household-scoped API routes — same signature and observable
 * behaviour as before Lot 0 (chantier foyer), but `householdId` is now resolved
 * in DB via the owner context (session → owner → memberships) instead of the
 * JWT's `hid`, which is vestigial. The single-membership invariant holds until
 * Lot 4; routes migrate to withOwnerAuth progressively in later lots.
 *
 * 401 when the session is unknown/revoked or has no membership; 403 when
 * `options.requireMember` is set and the resolved membership is a guest; any
 * uncaught error becomes a logged + Sentry-captured generic 500, so route
 * handlers only deal with their domain logic.
 */
export function withHouseholdAuth<Req extends Request, C, Res extends Response>(
  handler: (request: Req, context: C, auth: AuthContext) => Promise<Res>,
  options?: WithHouseholdAuthOptions,
) {
  // `context` is optional in the returned signature so tests can call
  // param-less handlers with a single argument; Next always passes both.
  return async (request: Req, context?: C): Promise<Res | NextResponse> => {
    try {
      // Dans le try : une erreur de résolution (DB indisponible) doit donner
      // un 500 capturé, PAS un 401 (cf. withOwnerAuth).
      const owner = await getOwnerContext();
      const membership = owner?.memberships[0];
      if (!owner || !membership) {
        return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
      }
      if (options?.requireMember && membership.role !== "member") {
        return NextResponse.json({ error: "Forbidden" }, { status: 403 });
      }
      return await handler(request, context as C, {
        householdId: membership.householdId,
        sessionId: owner.sessionId,
      });
    } catch (err) {
      Sentry.captureException(err);
      console.error(
        `[api] ${request.method} ${new URL(request.url).pathname}:`,
        err,
      );
      return NextResponse.json({ error: "Erreur serveur" }, { status: 500 });
    }
  };
}
