// Remise à zéro quotidienne des foyers démo (cron demo-reset, 03:00 UTC) —
// logique pure, hors HTTP (revue 2026-09-12 : sortie de la route pour être
// testable sans requête). La route ne garde que l'auth cron, la lecture des
...
function demoSeedMin(): number {
  ...
  if (!Number.isFinite(n) || n <= 0) {
    console.warn(
      `[demo/reset] DEMO_SEED_MIN invalide (« ${raw} ») — seuil ${DEFAULT_DEMO_SEED_MIN} appliqué`,
    );
...
export async function resetDemo(demoHouseholdIds: string[]): Promise<ResetSummary> {
  ...
  if (viewsError) {
    Sentry.captureException(
      new Error(`[cron/demo-reset] recipe_views purge failed: ${viewsError.message}`),
    );
  }
