...
describe("parseTsv", () => {
  ...
  it("saute l'en-tête répété quand plusieurs segments sont concaténés", () => {
    const rows = parseTsv(
      [
        DL_HEADER,
        dl("2026-09-09", "Redownload", "App referrer", 1),
        DL_HEADER,
...
describe("aggregateDownloads", () => {
  ...
  it("ignore les lignes sans date valide et les compteurs illisibles", () => {
    const rows = parseTsv(
      [
        DL_HEADER,
        dl("n/a", "First-time download", "App Store search", 9),
        dl("2026-09-09", "First-time download", "App Store search", NaN),
      ].join("\n"),
    );
    const agg = aggregateDownloads(rows);
    ...
    expect(agg.get("2026-09-08")).toEqual([
      {
        ...
        source_info: "",
        eng_impressions: 26,
        eng_impressions_uniq: 20,
        eng_page_views: 15,
        eng_page_views_uniq: 15,
        eng_taps: 0,
      },
      {
        source_type: "App referrer",
