...
describe("activation", () => {
  it("activée = ≥ 3 recettes ET retour après J+1", () => {
    expect(
      isActivated(person({ created_at: "2026-08-01", recipes_7d: 3, returned_7d: true })),
    ).toBe(true);
    expect(
      isActivated(person({ created_at: "2026-08-01", recipes_7d: 5, returned_7d: false })),
    ).toBe(false);
    expect(
      isActivated(person({ created_at: "2026-08-01", recipes_7d: 2, returned_7d: true })),
    ).toBe(false);
  });
  it("le parcours ne compte que les arrivées jugeables à J+7", () => {
    const people = [
      person({
        ...
        recipes_7d: 3,
        returned_7d: true,
        first_recipe_at: "2026-08-20T10:05:00Z",
      }),
      person({
        created_at: "2026-08-21T10:00:00Z",
        recipes_7d: 1,
        returned_7d: false,
        first_recipe_at: "2026-08-21T11:00:00Z",
      }),
...
describe("rétention", () => {
  ...
  it("courbes : cohortes ≥ 5 personnes avec M1 jugeable", () => {
    const people = Array.from({ length: 6 }, (_, i) =>
      person({ created_at: `2026-05-0${i + 1}`, active_m1: i < 3, active_m2: i < 1 }),
    );
    const rows = cohortTable(people, TODAY);
...
describe("acquisition & engagement", () => {
  it("nouvelles personnes par canal et par semaine", () => {
    ...
    expect(newPeople(people, { from: "2026-08-31", to: "2026-09-06" })).toEqual({
      ios: 1,
      android: 0,
      web: 1,
      invite: 0,
    ...
    expect(weekly.map((w) => [w.label, w.total])).toEqual([
      ["24/08", 0],
      ["31/08", 2],
    ]);
  });
