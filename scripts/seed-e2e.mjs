...
const { error: householdError } = await supabase.from("households").upsert(
  ...
  // rejoint (is_demo exclu de join/lookup), mais la colonne exige une valeur.
  {
    ...
    name: "Démo Mijote",
    join_code: "DEMO-0000",
    guest_join_code: "DEMOGUEST-0000",
    is_demo: true,
  },
  { onConflict: "id" },
);
...
if (DEMO_EN_ID) {
  const { error: enHouseholdError } = await supabase.from("households").upsert(
    {
      id: DEMO_EN_ID,
      name: "Mijote Demo",
      join_code: "DEMO-0001",
  ...
  console.log(
    `Seed E2E EN appliqué : foyer démo EN ${DEMO_EN_ID} + ${seedRecipesEn.length} recettes seed`,
  );
}
